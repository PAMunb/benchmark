...
contract KiddyToys is ERC721, Ownable {
    ...
    function getToy(uint256 _tokenId)
        public
        view
        returns (
            string toyName,
    ...
    function purchase(uint256 _tokenId) public payable {
        ...
        uint256 payment = uint256(
            SafeMath.div(SafeMath.mul(sellingPrice, 9), 10)
        ); //90% to previous owner
        uint256 win_payment = uint256(
            SafeMath.div(SafeMath.mul(sellingPrice, 9), 180)
        ); //5% to random owner
    ...
    function _createToy(
        ...
        Toy memory _toy = Toy({name: _name});
