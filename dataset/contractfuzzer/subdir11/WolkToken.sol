...
contract WolkToken {
    ...
    function settleFrom(
        ...
        if (
            balances[_from] >= _value &&
            (isPreauthorized || _allowance >= _value) &&
            _value > 0
        ) {
            balances[_to] = safeAdd(balances[_to], _value);
            balances[_from] = safeSub(balances[_from], _value);
    ...
    function checkAuthorization(address _owner, address _trustee)
        constant
        returns (bool authorization_status)
    {
    ...
    event Transfer(
        ...
        address indexed _to,
        uint256 _value,
        uint256 from_final_tok,
        uint256 to_final_tok
    );
    event Approval(
    ...
    event Deauthorization(
        address indexed _owner,
        address indexed _trustee_to_remove
    );

    event NewOwner(address _newOwner);
    event MintEvent(uint256 reward_tok, address recipient);
    ...
    modifier isOperational() {
        assert(saleCompleted);
        _;
    }
    modifier isTransferable() {
        assert(generalTokens > crowdSaleMin);
        _;
    }
    modifier is_not_dust() {
        if (msg.value < dust) throw;
        _;
    } // must be sufficiently large (1 mwei) to not be considered dust.
    ...
    // CROWDSALE SUPPORT
    function redeemToken() external payable is_not_dust {
    ...
    function mintTokens(uint256 reward_tok, address recipient)
        external
        payable
        onlyMinter
        isOperational
