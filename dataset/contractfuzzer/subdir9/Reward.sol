...
contract Reward {
    function reward(uint32[] rewardsDistribution, address[] winners) payable {
        ...
        uint256 forJack = msg.value;
        uint64 total = 0;
        for (uint8 j = 0; j < numWinners; j++) {
            // distribute all the winning even if there is not enought winners
            total += rewardsDistribution[j];
        }
        for (uint8 k = 0; k < numWinners; k++) {
            uint256 value = (msg.value * rewardsDistribution[k]) / total;
            if (winners[k].send(value)) {
                // skip winner if fail to send but still use next distribution index
