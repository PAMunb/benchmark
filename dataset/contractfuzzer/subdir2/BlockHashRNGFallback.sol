...
contract RNG {
     ...
     */
    function getRN(uint256 _block) public returns (uint256 RN);
    ...
    function getUncorrelatedRN(uint256 _block) public returns (uint256 RN) {
        ...
        else return uint256(keccak256(msg.sender, baseRN));
    }
}
...
contract BlockHashRNG is RNG {
     ...
     *  @param _block Block the random number is linked to.
     */
    function saveRN(uint256 _block) public {
...
contract BlockHashRNGFallback is BlockHashRNG {
    ...
    function saveRN(uint256 _block) public {
        if (_block < block.number && randomNumber[_block] == 0) {
            ...
            if (blockhash(_block) != 0x0)
                // Normal case.
                randomNumber[_block] = uint256(blockhash(_block)); // The contract was not called in time. Fallback to returning previous blockhash.
            else randomNumber[_block] = uint256(blockhash(block.number - 1));
        }
        if (randomNumber[_block] != 0) {
