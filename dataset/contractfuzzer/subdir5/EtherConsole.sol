...
contract EtherConsole is ERC721 {
    ...
    function createContractItem23(string _name, string _startingP)
        public
        onlyCOO
    {
    ...
    function stringToUint(string _amount)
        internal
        constant
    ...
    function getItem23(uint256 _tokenId)
        public
        view
        returns (
            string item23Name,
    ...
    function purchase(uint256 _tokenId) public payable {
        ...
        address[5] storage previousOwners = item23IndexToPreviousOwners[
            _tokenId
        ];
        ...
        uint256 ownerPayout = SafeMath.add(
            previousPrice,
            SafeMath.mul(SafeMath.div(priceDelta, 100), 40)
        );

        uint256 purchaseExcess = SafeMath.sub(msg.value, sellingPrice);

        item23IndexToPrice[_tokenId] = SafeMath.div(
            SafeMath.mul(sellingPrice, 150),
            100
        );
        item23IndexToPreviousPrice[_tokenId] = sellingPrice;

        uint256 strangePrice = uint256(
            SafeMath.mul(SafeMath.div(priceDelta, 100), 10)
        );
        ...
        for (uint256 i = 0; i < 5; i++) {
            if (previousOwners[i] != address(this)) {
                strangePrice2 += uint256(
                    SafeMath.mul(SafeMath.div(priceDelta, 100), 10)
                );
            } else {
                strangePrice = SafeMath.add(
                    strangePrice,
                    uint256(SafeMath.mul(SafeMath.div(priceDelta, 100), 10))
                );
            }
        }
    ...
    function _createItem23(
        ...
        Item23 memory _item23 = Item23({name: _name});
    ...
    function _transfer(
        ...
        item23IndexToPreviousOwners[_tokenId][3] = item23IndexToPreviousOwners[
            _tokenId
        ][2];
        item23IndexToPreviousOwners[_tokenId][2] = item23IndexToPreviousOwners[
            _tokenId
        ][1];
        item23IndexToPreviousOwners[_tokenId][1] = item23IndexToPreviousOwners[
            _tokenId
        ][0];
