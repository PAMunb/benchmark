...
contract EthronTokenPonzi {
    ...
    // CRR = 80 %
    int256 constant CRRN = 1;
    int256 constant CRRD = 2;
    // The price coefficient. Chosen such that at 1 token total supply
    // the reserve is 0.8 ether and price 1 ether/token.
    int256 constant LOGC = -0x296ABF784A358468C;

    string public constant name = "ETHRON";
    string public constant symbol = "ETRN";
    ...
    function withdraw(
        uint256 tokenCount // the parameter is ignored, yes
    ) public returns (bool) {
    ...
    function buy() internal {
        ...
        if (totalSupply > 0) {
            ...
            var holderreward = ((PRECISION -
                ...
                numEther) * (uint256)(CRRD)) / (uint256)(CRRD - CRRN);
