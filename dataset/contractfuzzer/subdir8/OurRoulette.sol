...
contract OurRoulette {
    struct Bet {
        uint256 value;
        uint256 height; //result of a bet placed at height is determined by blocks at height+1 and height+2, bet can be resolved from height+3 upwards..
        uint256 tier; //min bet amount
        bytes betdata;
    }
    mapping(address => Bet) bets;

    //helper function used when calculating win amounts
    function GroupMultiplier(uint256 number, uint256 groupID)
        public
        pure
        returns (uint256)
    {
        uint80[12] memory groups = [
            //matrix of bet multipliers for each group - 2bits per number
    ...
    function GetNumber(address adr, uint256 height)
        public
        view
        returns (uint256)
    {
        ...
        return ((uint256)(keccak256(adr, hash1, hash2))) % 37;
    }

    //returns user's payout from his last bet
    function BetPayout() public view returns (uint256 payout) {
        Bet memory tmp = bets[msg.sender];

        uint256 n = GetNumber(msg.sender, tmp.height);
        if (n == 69) return 0; //unable to get blockhash - too late

        payout = ((uint256)(tmp.betdata[n])) * 36; //if there is a bet on the winning number, set payout to the bet*36
        for (uint256 i = 37; i < 49; i++)
            payout += ((uint256)(tmp.betdata[i])) * GroupMultiplier(n, i - 37); //check all groups

        return payout * tmp.tier;
    }

    //claims last bet (if it exists), creates a new one and sends back any leftover balance
    function PlaceBet(uint256 tier, bytes betdata) public payable {
        Bet memory tmp = bets[msg.sender];
        uint256 balance = msg.value; //user's balance
        ...
        if (tmp.height != 0 && ((block.number - 1) >= (tmp.height + 2))) {
            //if there is an unclaimed bet that can be resolved...
            uint256 win = BetPayout();
        ...
        uint256 betsz = 0;
        for (uint256 i = 0; i < 49; i++) betsz += (uint256)(betdata[i]);
        ...
        )
    {
        return (
            ...
            bets[msg.sender].height,
            bets[msg.sender].tier,
            BetPayout(),
            bets[msg.sender].betdata
        );
    }
...
*/
    ...
    int256 constant crr_n = 1; // CRR numerator
    int256 constant crr_d = 2; // CRR denominator

    // The price coefficient. Chosen such that at 1 token total supply
    // the amount in reserve is 0.5 ether and token price is 1 Ether.
    int256 constant price_coeff = -0x296ABF784A358468C;
    ...
    function reinvestDividends() public {
        ...
        uint256 value_ = (uint256)(balance);

        // If your dividends are worth less than 1 szabo, or more than a million Ether
        // (in which case, why are you even here), abort.
        if (value_ < 0.000001 ether || value_ > 1000000 ether) revert();
        ...
        if (totalSupply > 0) {
            ...
            uint256 bonusCoEff = ((scaleFactor -
                ((res + numEther) * numTokens * scaleFactor) /
                (totalSupply + numTokens) /
                numEther) * (uint256)(crr_d)) / (uint256)(crr_d - crr_n);
        ...
        int256 payoutDiff = ((earningsPerToken * (int256)(numTokens)) -
            (int256)(buyerFee));
    ...
    {
        int256 r = ((earningsPerToken * (int256)(tokenBalance[_owner])) -
            payouts[_owner]) / (int256)(scaleFactor);
        if (r < 0) return 0;
        return (uint256)(r);
    }

    // Returns real dividends, including negative values
    function realDividends(address _owner)
        public
        constant
        returns (int256 amount)
    {
        return (((earningsPerToken * (int256)(tokenBalance[_owner])) -
            payouts[_owner]) / (int256)(scaleFactor));
    }
    ...
    function buy() internal {
        ...
        if (totalSupply > 0) {
            ...
            // same way they would have if they bought each token individually.
            uint256 bonusCoEff = ((scaleFactor -
                ((reserve() + numEther) * numTokens * scaleFactor) /
                (totalSupply + numTokens) /
                numEther) * (uint256)(crr_d)) / (uint256)(crr_d - crr_n);
    ...
    function sell(uint256 amount) internal {
        ...
        int256 payoutDiff = (earningsPerToken *
            (int256)(amount) +
            (int256)(numEthers * scaleFactor));
    ...
    function totalDiv() public view returns (int256) {
        return
            ((earningsPerToken * (int256)(totalSupply)) - totalPayouts) /
            (int256)(scaleFactor);
    }
    ...
    function getTokensForEther(uint256 ethervalue)
        public
        constant
        returns (uint256 tokens)
    {
        return
            sub(
                fixedExp(
                    (fixedLog(reserve() + ethervalue) * crr_n) /
                        crr_d +
                        price_coeff
                ),
                totalSupply
    ...
    function calculateDividendTokens(uint256 ethervalue, uint256 subvalue)
        public
        constant
        returns (uint256 tokens)
    {
        return
            sub(
                fixedExp(
                    (fixedLog(reserve() - subvalue + ethervalue) * crr_n) /
                        crr_d +
                        price_coeff
                ),
                totalSupply
    ...
    function getEtherForTokensOld(uint256 tokens)
        public
        constant
        returns (uint256 ethervalue)
    {
        ...
        if (tokens == totalSupply) return reserveAmount;
        ...
        return
            sub(
                reserveAmount,
                fixedExp(
                    ((fixedLog(totalSupply - tokens) - price_coeff) * crr_d) /
                        crr_n
                )
            );
    }
    ...
    function getEtherForTokens(uint256 tokens)
        public
        constant
        returns (uint256 ethervalue, uint256 penalty)
    {
