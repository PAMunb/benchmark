...
contract CryptoCarsRent is ERC721, Ownable {
    ...
    function getCar(uint256 _tokenId)
        public
        view
        returns (
            string carName,
    ...
    function purchase(uint256 _tokenId) public payable {
        ...
        if (renter != address(0)) {
            renter_payment = uint256(
                SafeMath.div(SafeMath.mul(profit, 45), 100)
            ); //45% from profit to car's renter
            payment = uint256(
                SafeMath.sub(
                    SafeMath.div(SafeMath.mul(sellingPrice, 97), 100),
                    renter_payment
                )
            ); //'97% - renter_payment' to previous owner
        } else {
            renter_payment = 0;
            payment = uint256(
                SafeMath.div(SafeMath.mul(sellingPrice, 94), 100)
            ); //94% to previous owner
        }
        ...
        if (sellingPrice < 500 finney) {
            ...
            carIdToPrice[_tokenId] = uint256(
                SafeMath.div(SafeMath.mul(sellingPrice, 15), 10)
            ); //rice by 50%
        }

        //plannig profit from next selling
        carIdToProfit[_tokenId] = uint256(
            SafeMath.sub(carIdToPrice[_tokenId], sellingPrice)
        );
        carIdToRenter[_tokenId] = address(0);
    ...
    function rent(uint256 _tokenId) public payable {
        ...
        uint256 rentPrice = uint256(
            SafeMath.div(SafeMath.mul(profit, 10), 100)
        ); //10% from profit is a rent price
        ...
        )
    {
        ...
        for (uint256 carId = 0; carId < totalResultCars; carId++) {
            ...
            else renters_res[carId] = address(0);
    ...
    function _createCar(
        ...
        Car memory _car = Car({name: _name});
    ...
    function _owns(address _checkedAddr, uint256 _tokenId)
        private
        view
        returns (bool)
    {
