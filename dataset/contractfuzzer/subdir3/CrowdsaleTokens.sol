...
// Interface
contract PriceModel {
    function getPrice(uint256 block) constant returns (uint256);
}
...
contract CrowdsaleTokens is SafeMath {
    ...
    // blocks
    uint256 public crowdsaleStarts; // block in which the ICO starts
    uint256 public crowdsaleEnds; // block in which the ICO ends

    // wei
    uint256 public totalCollected; // amount of wei collected

    // tokens
    uint256 public tokensIssued; // number of tokens (plus decimals) issued so far
    uint256 public tokenCap; // maximum number of tokens to be issued

    modifier crowdsalePeriod() {
        require(
            block.number >= crowdsaleStarts && block.number < crowdsaleEnds
        );
    ...
    function CrowdsaleTokens(
        ...
        uint256 _crowdsaleStarts,
        uint256 _crowdsaleEnds,
        uint256 _tokenCap
    ) {
    ...
    function buy() public payable crowdsalePeriod {
        // Calculate price
        uint256 price = calculatePrice(block.number);
    ...
    function processPurchase(uint256 price) private {
        // number of the tokens to be purchased  for the given price and ether sent
        uint256 numTokens = safeDiv(safeMul(msg.value, price), 1 ether);
        ...
        // mint tokens
        if (!MintInterface(tokenContract).mint(msg.sender, numTokens)) revert();
    }

    // Returns the number of tokens to be purchased by 1 ether at the given block
    function calculatePrice(uint256 block) public constant returns (uint256) {
        return PriceModel(priceModel).getPrice(block);
    }

    // Returns the number of tokens available for sale
    function remaining() public constant returns (uint256) {
