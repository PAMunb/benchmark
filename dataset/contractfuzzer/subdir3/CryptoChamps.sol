...
contract CryptoChamps is Ownable {
    ...
    event Birth(uint32 _id, uint256 _startingPrice);
    event Discount(uint32 _id, uint256 _newPrice);
    event Purchase(
        uint32 indexed _id,
        address indexed _by,
    ...
    mapping(address => uint256) private noOfPersonsOwned;
    ...
    function purchase(uint32 _id) public payable {
        uint256 price = personToPrice[_id];
        ...
        uint256 newPrice = _onPersonSale(person);
    ...
    function discount(uint32 _id, uint256 _newPrice)
        external
        ownsPerson(_id)
        returns (bool)
    {
    ...
    function _handlePurchase(
        Person storage _person,
        address _owner,
        uint256 _price
    ) internal {
        uint256 oldPrice = personToOldPrice[_person.id];
    ...
    function _shareDiscountPrice(uint256 _price, address _target) internal {
        uint256 commision = (_price * 10) / 100;
        _target.transfer(_price - commision);
        owner.transfer(commision);
    }

    function _shareProfit(
        uint256 _price,
        uint256 _oldPrice,
        address _target
    ) internal {
        uint256 profit = _price - _oldPrice;
        uint256 commision = (profit * 30) / 100;
        _target.transfer(_price - commision);
        owner.transfer(commision);
    }

    function _onPersonSale(Person storage _person) internal returns (uint256) {
        uint256 currentPrice = personToPrice[_person.id];
        uint256 percent = 0;
        ...
        uint256 newPrice = _approx((currentPrice * percent) / 100);
    ...
    function _approx(uint256 _price) internal pure returns (uint256) {
        uint256 product = _price / 10**14;
