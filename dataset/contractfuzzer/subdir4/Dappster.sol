...
contract Dappster {
    ...
    struct CardDetails {
        uint8 id;
        uint256 price;
        uint256 priceLease; // price per block
        uint256 leaseDuration; // in block
        bool availableBuy;
        bool availableLease;
        uint256[] leaseList;
        mapping(uint256 => LeaseCard) leaseCardStructs;
    }
    ...
    /// Initial card price
    uint256 public initialCardPrice = 0.1 ether;

    /// Owner cut (1%) . This cut only apply on a user-to-user card transaction
    uint256 public ownerBuyCut = 100;
    /// fluffyCat cut (10%)
    uint256 public fluffyCatCut = 1000;

    /// contractOwner can withdraw the funds
    address public contractOwner;
    /// fluffyCat address
    address public fluffyCatAddress =
        0x2c00A5013aA2E600663f7b197C98db73bA847e6d;
    ...
    function initialBuyCard(
        uint8 cardId,
        string title,
        string url,
        string image
    ) public payable onlyValidCard(cardId) returns (bool success) {
        // Check sent amount
        uint256 price = computeInitialPrice(cardId);
    ...
    function buyCard(
        ...
        // Check sent amount
        uint256 price = cardDetailsStructs[cardId].price;
        require(msg.value >= price);
    ...
    function sellCard(uint8 cardId, uint256 price)
        public
        onlyValidCard(cardId)
        onlyCardOwner(cardId)
        returns (bool success)
    ...
    function cancelSellCard(uint8 cardId)
        public
        onlyValidCard(cardId)
        onlyCardOwner(cardId)
        returns (bool success)
        ...
        returns (bool success)
    {
        ...
        uint256 _lastLeaseId = getCardLeaseLength(cardId);
        uint256 _until = cardDetailsStructs[cardId]
            .leaseCardStructs[_lastLeaseId]
            .untilBlock;
    ...
    function cancelLeaseOffer(uint8 cardId)
        public
        onlyValidCard(cardId)
        onlyCardOwner(cardId)
        returns (bool success)
    ...
    function leaseCard(
        ...
        uint256 price = details.priceLease;
        uint256 leaseDuration = details.leaseDuration;
        uint256 totalAmount = price * leaseDuration;
        // Check that amount sent is sufficient
        require(msg.value >= totalAmount);
        // Get new lease id
        uint256 leaseId = getCardLeaseLength(cardId) + 1;
        // Get the block number of lease end
        uint256 untilBlock = block.number + leaseDuration;
        // Take 1% cut on lease
        Card storage card = cardStructs[cardId];
    ...
    {
        uint256 _leaseIndex = getCardLeaseLength(cardId);
        return getLease(cardId, _leaseIndex);
    }

    /// Get lease from card
    function getLease(uint8 cardId, uint256 leaseId)
        public
        view
        returns (
            uint256 leaseIndex,
    ...
    function getCardLeaseLength(uint8 cardId)
        public
        view
        returns (uint256 cardLeasesCount)
    {
    ...
    function computeInitialPrice(uint8 cardId)
        public
        view
        onlyValidCard(cardId)
        returns (uint256 price)
    {
        ...
        return
            initialCardPrice -
            ((initialCardPrice / 100) * (uint256(cardId) - 1));
    }

    /// Allow contract owner to set NSFW flag on a card
    function setNSFW(uint8 cardId, bool flag)
        public
        onlyValidCard(cardId)
        onlyContractOwner
        returns (bool success)
    ...
    function _initCardDetails(uint8 cardId, uint256 price)
        internal
        returns (bool success)
    {
    ...
    function _computeCut(uint256 _price, uint256 _cut)
        internal
        pure
        returns (uint256)
    {
